...
range_polygon = {
    'Range 62B': [
        (-330815, -64799), (-330429, -42521), (-345241, -42248), (-345428, -53325),
        (-341718, -53355), (-341822, -59264), (-345518, -59204), (-345592, -64590)
    ]
}
...
def area_control_tracker():
    # check player's position?
    for export_id, unit_data in cdi.unit_precise_positions.items():
        if unit_data['Flags']['Human'] is True:  # is a player controlled unit, check position
            pos = unit_data['Position']
            if point_in_range(pos['x'], pos['z'], 'Range 62B'):  # if unit position is in this range
                pass
...
def point_in_range(x, y, range_id):
    q_point = Point(x, y)
    range_boundary_point = range_polygon[range_id]
    poly = Polygon(range_boundary_point)

    if poly.contains(q_point):
        return True
    else:
        return False
...
def report_valid_data_when_out_of_range(group_id, range_name):
    while True:
        if group_in_range(group_id, range_name):
            print(f"{range_name}")
            # still in range, continue
        else:  # player not in range anymore, send data
            print(f"group {group_id} is no longer in range {range_name}")
            break
